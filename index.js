...
const verifyJWT = (req, res, next) => {
    ...
    if (!authorization) {
        return res.status(401).send({error: true, message: "unauthenticated access"});
    }
    // bearer token
    const token = authorization.split(' ')[1];
    jwt.verify(token, process.env.ACCESS_TOKEN_SECRET, (err, decoded) => {
        if (err) {
            return res.status(500).send({error: true, message: "unauthenticated access"});
        }
        req.decoded = decoded;
        next();
    })
...
async function run() {
    try {
        ...
        // verify jwt before using verifyAdmin
